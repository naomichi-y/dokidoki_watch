...
let fitbitApiHelper = {
    heartrateEndpoint: timezone => {
        ...
        let beginOffset = config.heartrate.monitor.minutes_range + config.heartrate.monitor.minutes_ago - 1
    ...
    heartrateDataset: user => {
        return new Promise((resolve, reject) => {
            let endpoint = fitbitApiHelper.heartrateEndpoint(user.timezone)
