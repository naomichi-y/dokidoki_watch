        ...
        getHeartrateDataset(user).then(results => {
            ...
            if (dataset.length) {
                ...
                dataset.forEach(data => {
                    sum += data.value
                    currentDate = moment(`${activitiesHeart.dateTime} ${data.time}`).tz(user.timezone)
                    ...
                    bulk.push({
                        user_id: user.id,
                        bpm: data.value,
                        activity_at: currentDate.format('YYYY-MM-DD HH:mm:ss')
                    })

                    previousDate = moment(`${activitiesHeart.dateTime} ${data.time}`).tz(user.timezone)
                })
